...
# Step 1: Transform the phyloseq object to relative abundance
nasa_RA <- transform_sample_counts(nasa_rare, function(x) x / sum(x))

# Step 2: Prepare the OTU table and grouping vector
cat("Extracting OTU matrix and group vector...\n")
otu_matrix <- t(otu_table(nasa_RA))
group_vector <- sample_data(nasa_RA)$humidity_bin  # Adjust this to your grouping variable

# Step 3: Perform Indicator Species Analysis (ISA)
cat("Performing ISA...\n")
isa_output <- multipatt(otu_matrix, cluster = group_vector)

# Step 4: Extract significant ASVs from the ISA output directly
cat("Extracting significant ASVs...\n")
significant_ASVs <- rownames(isa_output$sign[isa_output$sign$p.value <= 0.05, , drop = FALSE])
cat("Number of significant ASVs found:", length(significant_ASVs), "\n")

# Check if significant ASVs are found
if (length(significant_ASVs) == 0) {
  stop("No significant ASVs identified.")
}

# Step 5: Extract abundance data for significant ASVs
extract_abundance <- function(physeq_obj, significant_ASVs) {
  # Extract OTU table as a data frame
  otu_df <- as.data.frame(otu_table(physeq_obj))
  otu_df$ASV <- rownames(otu_df)
  
  # Filter for significant ASVs and pivot to long format
  abundance_df <- otu_df %>%
    filter(ASV %in% significant_ASVs) %>%
  ...
  merged_df <- left_join(abundance_df, taxonomy_df, by = "ASV")
  
  # Extract sample data and convert row names to a column named "SampleID"
  sample_df <- as.data.frame(sample_data(physeq_obj))
  sample_df$SampleID <- rownames(sample_df)
  
  # Merge with sample data using SampleID
  final_df <- left_join(merged_df, sample_df, by = "SampleID")
  
  # Check if the resulting dataframe is empty
  if (nrow(final_df) == 0) {
    stop("No matching data found between abundance and sample data.")
  }
  
  return(final_df)
}

# Step 6: Extract abundance data for significant ASVs
cat("Extracting abundance data for significant ASVs...\n")
significant_df <- extract_abundance(nasa_RA, significant_ASVs)

if (nrow(significant_df) == 0) {
  stop("No data found for significant ASVs.")
}

# Step 7: Split the dataframe by humidity bin
low_df <- significant_df %>% filter(humidity_bin == "Low")
medium_df <- significant_df %>% filter(humidity_bin == "Medium")
high_df <- significant_df %>% filter(humidity_bin == "High")

# Step 8: Define the plotting function
plot_abundance <- function(df, title) {
  ggplot(df, aes(x = SampleID, y = Abundance, fill = Genus)) +
    ...
    ylim(0, 0.1) # Set a fixed y-axis limit for consistency
}

# Step 9: Create individual plots for each humidity bin
low_plot <- plot_abundance(low_df, "Low Humidity") +
  theme_minimal() +
  theme(
    axis.title.x = element_blank(),
    axis.text.x = element_blank(),
...
medium_plot <- plot_abundance(medium_df, "Medium Humidity") +
  theme_minimal() +
  theme(
    axis.title.x = element_blank(),
    axis.text.x = element_blank(),
...
high_plot <- plot_abundance(high_df, "High Humidity")

# Step 10: Combine plots using patchwork to align them vertically
combined_plot <- low_plot / medium_plot / high_plot +
  plot_layout(ncol = 1) + 
  plot_annotation(
    title = "Abundance of Indicator Species by Humidity Bin",
    theme = theme(plot.title = element_text(size = 16, face = "bold"))
  )

# Step 11: Display the combined plot
print(combined_plot)