# This is the code to do AIM 1, it includes code to do alpha and beta diversity metrics
...
### Figure 1.a: Alpha Diversity comparison of metadata treatment column ----
    ...
    gg_richness <- plot_richness(
     ...
     geom_boxplot(aes(fill = treatment), alpha = 0.3, outlier.shape = 21, outlier.color = "black", outlier.size = 2) +
     scale_fill_manual(values = c("Constant Relative Humidity" = "#FF5733", "Varying Relative Humidity" = "#33FF57")) +
    ...
    plot_faith <- ggplot(sample_data(nasa_rare), aes(x = treatment, y = Faith_PD, fill = treatment)) +
...
### Figure 1.b ----
    ...
    gg_pcoa <- plot_ordination(nasa_rare, pcoa_bc, color = "treatment") +
      ...
      scale_color_manual(values = c("Constant Relative Humidity" = "#FF5733", "Varying Relative Humidity" = "#33FF57")) +
      scale_fill_manual(values = c("Constant Relative Humidity" = "#FF5733", "Varying Relative Humidity" = "#33FF57")) +
...
### Figure 1.c ---- 
    ...
    gg_taxa <- ggplot(nasa_phylum_data, aes(x = treatment, y = mean_abundance, fill = Phylum)) +
